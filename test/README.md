...
本项目使用 Node.js 内置的测试框架进行测试。

### 测试框架配置

- **测试运行器**: Node.js Test Runner (内置)
- **断言库**: Node.js 内置 assert 模块
- **属性测试**: fast-check
- **测试命令**: `node --test`
...
# 运行特定测试文件
node --test test/platform/runtime.test.js

# 运行匹配模式的测试
node --test --test-name-pattern "Property"
```
...
- [Node.js Test 文档](https://nodejs.org/docs/latest/api/test.html)
