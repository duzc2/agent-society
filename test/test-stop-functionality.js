...
async function testStopFunctionality() {
  ...
  console.log(`  重复停止结果: ok=${stopAgainResult.ok}, reason=${/** @type {any} */(stopAgainResult).reason}`);
  
  // @ts-ignore - reason 属性不存在于返回类型中
  ...
  // 级联停止父智能体
  // @ts-ignore - abortAgentLlmCall 只接受一个参数
  const cascadeResult = await runtime.abortAgentLlmCall(parentAgent.id, true);
  console.log(`  级联停止结果: ok=${cascadeResult.ok}, cascadeStopped=${/** @type {any} */(cascadeResult).cascadeStopped?.length ?? 0}`);
