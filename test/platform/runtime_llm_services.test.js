...
{"serviceId": "选中的服务ID", "reason": "选择原因"}`);
  ...
  describe("Property 8: LlmClient 池实例复用", () => {
    it("首次调用 getLlmClientForService 应创建新实例，后续调用相同 ID 应返回相同实例引用", async () => {
      ...
      // 初始？Runtime
      runtime = new Runtime({ configPath: path.join(configDir, "app.json") });
      await runtime.init();

      // 验证服务注册表已加载
      assert.notStrictEqual(runtime.serviceRegistry, null);
      assert.strictEqual(await runtime.serviceRegistry.hasServices(), true);

      // 首次调用 - 应创建新实例
      const client1 = await runtime.getLlmClientForService("service-a");
      assert.notStrictEqual(client1, null);
      assert.ok(client1 instanceof LlmClient);

      // 再次调用相同 ID - 应返回相同实例
      const client2 = await runtime.getLlmClientForService("service-a");
      assert.strictEqual(client2, client1); // 严格相等，同一引用

      // 调用不同 ID - 应创建新实例
      const client3 = await runtime.getLlmClientForService("service-b");
      assert.notStrictEqual(client3, null);
      assert.notStrictEqual(client3, client1); // 不同实例

      // 再次调用 service-b - 应返回相同实例
      const client4 = await runtime.getLlmClientForService("service-b");
      assert.strictEqual(client4, client3);

      // 验证池中有两个实例
      assert.strictEqual(runtime.llmClientPool.size, 2);
    });
      ...
      await fc.assert(
        fc.asyncProperty(
          ...
          async (services) => {
            ...
            const rt = new Runtime({ configPath: path.join(configDir, "app.json") });
            try {
              await rt.init();

              // 对每个服务 ID 多次调用，验证返回相同实例
              for (const service of uniqueServices) {
                const firstCall = await rt.getLlmClientForService(service.id);
                const secondCall = await rt.getLlmClientForService(service.id);
                const thirdCall = await rt.getLlmClientForService(service.id);

                assert.notStrictEqual(firstCall, null);
                assert.strictEqual(secondCall, firstCall);
                assert.strictEqual(thirdCall, firstCall);
              }
            } finally {
              await rt.shutdown();
            }
    ...
    it("不存在的服务 ID 也会创建客户端（源代码行为）", async () => {
      const servicesConfig = {
        services: [
          {
            id: "existing-service",
      ...
      // 源代码行为：不为 null 的 serviceId 会创建客户端（不检查配置存在性）
      // 只有 null/undefined/空字符串返回 null
      const client = await runtime.getLlmClientForService("non-existent-service");
      assert.notStrictEqual(client, null); // 源代码会为任意非空 serviceId 创建客户端
      assert.ok(client instanceof LlmClient);

      // 空字符串返回 null
      const client2 = await runtime.getLlmClientForService("");
      assert.strictEqual(client2, null);

      // null 返回 null
      const client3 = await runtime.getLlmClientForService(null);
      assert.strictEqual(client3, null);
    });
  });
  ...
  describe("Property 9: LlmClient 回退逻辑", () => {
    it("岗位未指定 llmServiceId 时应使用默认 LlmClient", async () => {
      ...
      // 获取智能体的 LlmClient
      const client = await runtime.getLlmClientForAgent(agentMeta.id);

      // 应该返回默认 LlmClient
      assert.strictEqual(client, runtime.llm);
    });
    ...
    it("岗位指定的 llmServiceId 存在时应返回对应的 LlmClient", async () => {
      ...
      assert.notStrictEqual(client, runtime.llm);
      assert.ok(client instanceof LlmClient);

      // 验证是从池中获取的
      assert.strictEqual(runtime.llmClientPool.has("coding-service"), true);
      assert.strictEqual(runtime.llmClientPool.get("coding-service"), client);
    });

    it("岗位指定的 llmServiceId 不存在时也会创建新客户端（源代码行为）", async () => {
      ...
      // 源代码行为：getLlmClientForService 会为任意 serviceId 创建客户端
      // 所以不会回退到默认，而是返回新创建的客户端
      assert.notStrictEqual(client, runtime.llm);
      assert.ok(client instanceof LlmClient);
    });
    ...
    it("智能体不存在时应返回默认 LlmClient", async () => {
      ...
      const client = await runtime.getLlmClientForAgent("non-existent-agent");
    ...
    it("agentId 为空时应返回默认 LlmClient", async () => {
      ...
      assert.strictEqual(await runtime.getLlmClientForAgent(null), runtime.llm);
      assert.strictEqual(await runtime.getLlmClientForAgent(undefined), runtime.llm);
      assert.strictEqual(await runtime.getLlmClientForAgent(""), runtime.llm);
    });
        ...
        fc.asyncProperty(
          ...
          async ({ hasService, serviceId, roleHasServiceId, roleServiceIdMatches, uniqueSuffix }) => {
            ...
            try {
              ...
              const role = await rt.org.createRole({
              ...
              // 创建智能体
              const agentMeta = await rt.org.createAgent({
                roleId: role.id,
                parentAgentId: "root"
              });

              rt._agents.set(agentMeta.id, {
                id: agentMeta.id,
                roleId: role.id,
                roleName: role.name
              });
              ...
              // 获取 LlmClient
              const client = await rt.getLlmClientForAgent(agentMeta.id);

              // 验证行为（源代码：getLlmClientForService 不为 null 时返回客户端）
              if (!roleHasServiceId || !roleLlmServiceId) {
                // 岗位未指定 llmServiceId，应使用默认
                assert.strictEqual(client, rt.llm);
              } else {
                // 岗位指定了 llmServiceId，源代码会为任意 serviceId 创建客户端
                // 所以总是返回新客户端，不回退到默认
                assert.notStrictEqual(client, rt.llm);
                assert.ok(client instanceof LlmClient);
              }
            } finally {
              await rt.shutdown();
