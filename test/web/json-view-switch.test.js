    ...
    fc.assert(
      fc.property(
        ...
        (obj) => {
          ...
          assert.strictEqual(switcher.renderedMode, "json");
          assert.strictEqual(switcher.jsonDisplayMode, "json");
          assert.ok(switcher.viewerPanel.innerHTML.includes("json-tree"));
    ...
    fc.assert(
      fc.property(
        ...
        (obj) => {
          ...
          assert.strictEqual(switcher.renderedMode, "text");
          assert.strictEqual(switcher.jsonDisplayMode, "text");
          assert.ok(switcher.viewerPanel.innerHTML.includes("json-text-content"));
      ...
      fc.property(
        ...
        (obj, modes) => {
          ...
          for (const mode of modes) {
            ...
            assert.strictEqual(switcher.jsonDisplayMode, mode);
            assert.strictEqual(switcher.renderedMode, mode);
          }
    ...
    fc.assert(
      fc.property(
        ...
        (obj) => {
          ...
          // 内容应保持不变
          assert.strictEqual(JSON.stringify(switcher.currentJsonContent), originalContent);
          assert.strictEqual(switcher.currentJsonRaw, originalRaw);
...
describe('功能: json-artifact-viewer-enhancement, 文本视图截断测试', () => {
  ...
  it('超过5000字符的内容应被截断', () => {
    ...
    // 由于是HTML，实际内容在pre标签内
    assert.ok(renderedContent.length < switcher.currentJsonRaw.length + 100);
  });
      ...
      fc.property(
        ...
        (obj) => {
          ...
          if (jsonStr.length <= 5000) {
            // 内容应完整显示
            assert.ok(switcher.viewerPanel.innerHTML.includes(jsonStr.substring(0, 50)));
          }
