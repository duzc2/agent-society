...
npx c8 --reporter=lcov node --test

# 指定覆盖率报告目录
npx c8 --reporter=lcov --output-dir=./coverage node --test
```
...
1. **text**: 控制台文本输出（默认，通过 `--experimental-test-coverage`）
2. **lcov**: LCOV 格式，用于 CI/CD 集成（通过 `c8` 工具）
...
注意：可通过 c8 配置文件 `.c8rc.json` 排除不需要覆盖的文件。
...
### Q: 如何查看详细的覆盖率报告？
A: 使用 `npx c8 --reporter=lcov node --test` 生成 LCOV 报告，然后使用 LCOV 查看器查看。
...
- [Node.js Test Coverage](https://nodejs.org/docs/latest/api/test.html#coverage-reporting)
