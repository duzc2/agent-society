...
import { readFile, access, mkdir, appendFile } from "node:fs/promises";
 ...
 * 外部路径访问日志记录器类
 */
export class ExternalAccessLogger {
  /**
   * @param {{logDir: string, configManager: any, log: any}} options
   */
  constructor(options) {
    this.logDir = options.logDir;
    this.configManager = options.configManager;
    this._logger = options.log;
  ...
  async init() {
    ...
    this._logger.info("[WorkspaceFileAccess] 访问日志记录器初始化完成", {
      logDir: this.logDir
    });
  ...
  async log(entry) {
    try {
      ...
      this._logger.error("[WorkspaceFileAccess] 记录访问日志失败", {
        error: error.message,
        stack: error.stack,
        entry
      });
   ...
   * @private
   * @param {object} ctx
   * @returns {{orgId: string|null, orgName: string|null}}
   */
  _getOrgInfo(ctx) {
    const orgId = ctx?.org?.id ?? ctx?.agent?.orgId ?? null;
    ...
    try {
      ...
      for (const filename of logFiles) {
        ...
        try {
          ...
          this._logger.error("[WorkspaceFileAccess] 读取日志文件失败", {
            file: filename,
            error: error.message,
            stack: error.stack
          });
      ...
      this._logger.error("[WorkspaceFileAccess] 加载日志文件失败", {
        error: error.message,
        stack: error.stack
      });
    }
  ...
  async cleanupOldLogs() {
    const retentionDays = this.configManager.getLogRetentionDays();
    ...
    try {
      ...
      this._logger.info("[WorkspaceFileAccess] 清理过期日志完成", {
        deleted,
        retentionDays
      });

    } catch (error) {
      this._logger.error("[WorkspaceFileAccess] 清理过期日志失败", {
        error: error.message,
        stack: error.stack
      });
    }
...
export default ExternalAccessLogger;
