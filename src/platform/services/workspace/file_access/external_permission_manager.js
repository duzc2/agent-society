 ...
 * 外部路径权限管理器类
 */
export class ExternalPermissionManager {
  /**
   * @param {{configManager: any, log: any}} options
   */
  constructor(options) {
  ...
  async checkReadPermission(filePath, orgId = null) {
    try {
      ...
      this.log.error("[WorkspaceFileAccess] 检查读取权限失败", {
        path: filePath,
        error: error.message,
        stack: error.stack
      });
  ...
  async checkWritePermission(filePath, orgId = null) {
    try {
      ...
      this.log.error("[WorkspaceFileAccess] 检查写入权限失败", {
        path: filePath,
        error: error.message,
        stack: error.stack
      });
  ...
  _normalizePath(filePath) {
    ...
    try {
      ...
      this.log.error("[WorkspaceFileAccess] 路径规范化失败", {
        path: filePath,
        error: error.message,
        stack: error.stack
      });
...
export default ExternalPermissionManager;
