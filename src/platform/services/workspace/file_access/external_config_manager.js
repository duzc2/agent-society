...
const MODULE_NAME = "workspace_file_access";
...
export class ExternalConfigManager {
  ...
  /**
   * 初始化：注册默认值 → 与 config/modules/workspace_file_access.json 合并 → 应用到实例
   */
  async init() {
    if (this._initialized) return;

    this.configService.registerModuleConfig(MODULE_NAME, DEFAULTS);

    const merged = await this.configService.getModuleConfig(MODULE_NAME);
    this.folders = Array.isArray(merged.folders) ? [...merged.folders] : [];
    this.logRetentionDays =
      typeof merged.logRetentionDays === "number" && !isNaN(merged.logRetentionDays) && merged.logRetentionDays >= 1
    ...
    this.log.info("[WorkspaceFileAccess] 外部路径配置管理器初始化完成", {
  ...
  async _persist() {
    try {
      ...
      await this.configService.saveModuleConfig(MODULE_NAME, {
        folders: this.folders,
        logRetentionDays: this.logRetentionDays,
        orgConfigs: orgConfigsObj
      });
    } catch (error) {
      this.log.error("[WorkspaceFileAccess] 保存配置失败", {
        error: error.message,
        stack: error.stack
      });
      throw error;
  ...
  async addFolder(folderConfig) {
    try {
      ...
      this.log.info("[WorkspaceFileAccess] 添加授权文件夹", {
        folderId: folder.id,
        path: folder.path
      });
      ...
      this.log.error("[WorkspaceFileAccess] 添加文件夹失败", {
        error: error.message,
        stack: error.stack
      });
  ...
  async updateFolder(folderId, updates) {
    try {
      ...
      this.log.info("[WorkspaceFileAccess] 更新授权文件夹", { folderId });

      return { ok: true, folder: { ...folder } };

    } catch (error) {
      this.log.error("[WorkspaceFileAccess] 更新文件夹失败", {
        error: error.message,
        stack: error.stack
      });
      return { ok: false, error: "save_failed" };
  ...
  async removeFolder(folderId) {
    try {
      ...
      this.log.info("[WorkspaceFileAccess] 删除授权文件夹", {
        folderId,
        path: folder.path
      });
      ...
      this.log.error("[WorkspaceFileAccess] 删除文件夹失败", {
        error: error.message,
        stack: error.stack
      });
      return { ok: false, error: "save_failed" };
  ...
  async setLogRetentionDays(days) {
    try {
      ...
      this.log.error("[WorkspaceFileAccess] 设置日志保留天数失败", {
        error: error.message,
        stack: error.stack
      });
      return { ok: false, error: "save_failed" };
  ...
  async addFolderToOrg(orgId, folderConfig) {
    try {
      ...
      this.log.info("[WorkspaceFileAccess] 为组织添加授权文件夹", {
        orgId,
        folderId: folder.id,
        path: folder.path
      });
      ...
      this.log.error("[WorkspaceFileAccess] 为组织添加文件夹失败", {
        orgId,
        error: error.message,
        stack: error.stack
      });
  ...
  async updateFolderInOrg(orgId, folderId, updates) {
    try {
      ...
      this.log.info("[WorkspaceFileAccess] 更新组织授权文件夹", { orgId, folderId });

      return { ok: true, folder: { ...folder } };

    } catch (error) {
      this.log.error("[WorkspaceFileAccess] 更新组织文件夹失败", {
        orgId,
        folderId,
        error: error.message,
        stack: error.stack
  ...
  async removeFolderFromOrg(orgId, folderId) {
    try {
      ...
      this.log.info("[WorkspaceFileAccess] 从组织移除授权文件夹", { orgId, folderId });

      return { ok: true };

    } catch (error) {
      this.log.error("[WorkspaceFileAccess] 从组织移除文件夹失败", {
        orgId,
        folderId,
        error: error.message,
        stack: error.stack
  ...
  async setLogRetentionDaysForOrg(orgId, days) {
    try {
      ...
      this.log.info("[WorkspaceFileAccess] 设置组织日志保留天数", { orgId, days: numDays });

      return { ok: true };

    } catch (error) {
      this.log.error("[WorkspaceFileAccess] 设置组织日志保留天数失败", {
        orgId,
        error: error.message,
        stack: error.stack
      });
  ...
  async removeOrgConfig(orgId) {
    try {
      if (this.orgConfigs.has(orgId)) {
        ...
        await this._persist();
        this.log.info("[WorkspaceFileAccess] 移除组织配置", { orgId });
      }
      return { ok: true };

    } catch (error) {
      this.log.error("[WorkspaceFileAccess] 移除组织配置失败", {
        orgId,
        error: error.message,
        stack: error.stack
      });
...
export default ExternalConfigManager;
