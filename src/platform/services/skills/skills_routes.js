 ...
 * 标准化技能绑定数组，过滤无效条目。
 * @param {any} bindings
 * @returns {Array<{skillId:string, enabled:boolean}>|null}
 */
...
/**
 * 注册技能相关的所有 HTTP 路由端点。
 * @param {{app:any, skillsService:any, customSkillService:any, gitSkillService:any, logRoot:any}} deps
 */
function registerSkillRoutes({ app, skillsService, customSkillService, gitSkillService, logRoot }) {
  const log = logRoot.forModule('skill-routes');
  ...
  app.post('/api/custom-skills', async (c) => {
    ...
    try {
      ...
      void log.error("创建自定义技能失败", { error: createError?.message ?? String(createError), stack: createError?.stack });
  ...
  app.post('/api/custom-skills/copy', async (c) => {
    ...
    try {
      ...
      void log.error("复制自定义技能失败", { error: copyError?.message ?? String(copyError), stack: copyError?.stack });
  ...
  app.post('/api/custom-skills/:skillId/file', async (c) => {
    ...
    try {
      ...
      void log.error("创建自定义技能文件失败", { skillId, path: body?.path, error: createError?.message ?? String(createError), stack: createError?.stack });
  ...
  app.put('/api/custom-skills/:skillId/file', async (c) => {
    ...
    try {
      ...
      void log.error("写入自定义技能文件失败", { skillId, path: body?.path, error: writeError?.message ?? String(writeError), stack: writeError?.stack });
  ...
  app.post('/api/custom-skills/:skillId/folder', async (c) => {
    ...
    try {
      ...
      void log.error("创建自定义技能文件夹失败", { skillId, path: body?.path, error: createError?.message ?? String(createError), stack: createError?.stack });
      const statusCode = createError?.message === "custom_skill_not_found" ? 404 : 400;
  ...
  app.delete('/api/custom-skills/:skillId/entry', async (c) => {
    ...
    try {
      ...
      void log.error("删除自定义技能条目失败", { skillId, entryPath, error: deleteError?.message ?? String(deleteError), stack: deleteError?.stack });
  ...
  app.post('/api/custom-skills/:skillId/entry/rename', async (c) => {
    ...
    try {
      ...
      void log.error("重命名自定义技能条目失败", { skillId, fromPath: body?.fromPath, toPath: body?.toPath, error: renameError?.message ?? String(renameError), stack: renameError?.stack });
  ...
  app.post('/api/custom-skills/:skillId/status', async (c) => {
    ...
    try {
      ...
      void log.error("设置自定义技能状态失败", { skillId, status: body?.status, error: statusError?.message ?? String(statusError), stack: statusError?.stack });
  ...
  app.post('/api/git-skills/import', async (c) => {
    ...
    try {
      ...
      void log.error("Git技能导入失败", { error: importError?.message ?? String(importError), stack: importError?.stack });
  ...
  app.get('/api/git-skills/:skillId/file', async (c) => {
    ...
    try {
      ...
      void log.error("读取Git技能文件失败", { skillId, filePath, error: readError?.message ?? String(readError), stack: readError?.stack });
  ...
  app.post('/api/git-skills/:skillId/status', async (c) => {
    ...
    try {
      ...
      void log.error("设置Git技能状态失败", { skillId, status, error: statusError?.message ?? String(statusError), stack: statusError?.stack });
      return c.json({ error: "status_update_failed", message: statusError?.message || "更新状态失败" }, 500);
    }
  });
}

export { normalizeSkillBindings, registerSkillRoutes };
