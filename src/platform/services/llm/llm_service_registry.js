...
function validateServiceConfig(service) {
  ...
  if (
    ...
    errors.push(`服务provider必须是以下值之一: ${SUPPORTED_LLM_PROVIDERS.join(", ")}`);
  }
 ...
 * @property {boolean} enabled - 是否启用服务 (默认 true)
 */
...
export class LlmServiceRegistry {
  ...
  async getAllServices() {
    return this._loadAllServices();
  }

  /**
   * 加载并验证所有服务（内部方法，不进行 enabled 过滤）
   * @returns {Promise<LlmServiceConfig[]>}
   * @private
   */
  async _loadAllServices() {
    let servicesArray = [];
    ...
    try {
      ...
      void this.log.error("从 ConfigService 获取服务失败", { error: err.message, stack: err.stack });
    }
    ...
    if (servicesArray.length === 0) {
      ...
      if (configPath) {
        try {
          ...
          void this.log.error("读取 LLM 服务配置文件失败", { configPath, error: err.message, stack: err.stack });
        }
      }
    }
