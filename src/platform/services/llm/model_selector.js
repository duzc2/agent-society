...
export class ModelSelector {
  ...
  async selectService(rolePrompt) {
    // 如果服务注册表为空，跳过选择流程
    if (!(await this.serviceRegistry.hasServices())) {
    ...
    try {
      ...
      void this.log.error("模型选择过程发生异常", {
        // 业务信息：为哪个 role 选择模型时出错
        rolePromptLength: rolePrompt?.length ?? 0,
        rolePromptPreview: rolePrompt?.substring(0, 300) ?? null,
        servicesCount: services.length,
        serviceIds: services.map(s => s.id),
        llmCallCount: this._llmCallCount,
        // 触发参数：发送给 LLM 的 selection prompt 和返回的响应
        promptPreview: prompt?.substring(0, 500) ?? null,
        responseContent: typeof response?.content === 'string' ? response.content.substring(0, 300) : null,
        // 技术信息：异常详情
        error: errorMessage,
        stack: err?.stack,
        name: err?.name,
