...
export class MessageProcessor {
  ...
  async scheduleMessageProcessing(maxConcurrent) {
    ...
    for (const agentId of runtime._agents.keys()) {
      ...
      // 异步处理消息（带超时保护，防止 LLM 调用无限挂起导致 agent 永久锁定）
      const PROCESS_TIMEOUT_MS = 5 * 60 * 1000;
      const processPromise = this.processAgentMessage(agentId, msg);
      const timeoutPromise = new Promise((_, reject) =>
        setTimeout(() => reject(new Error(`智能体 ${agentId} 处理消息超时（${PROCESS_TIMEOUT_MS}ms）`)), PROCESS_TIMEOUT_MS)
      );
      Promise.race([processPromise, timeoutPromise])
        .catch((err) => {
          void runtime.log.error("智能体消息处理异常", {
            agentId,
            messageId: msg.id,
            error: err?.message,
            stack: err?.stack,
  ...
  async processAgentMessage(agentId, msg) {
    ...
    if (!agent) {
      void runtime.log.warn("智能体不存在，跳过消息处理", {
        agentId,
        messageId: msg.id,
        from: msg.from,
        taskId: msg.taskId
    ...
    // 记录生命周期事件
    void runtime._agentManager.logLifecycleEvent("agent_message_received", {
      agentId,
      messageId: msg.id ?? null,
      from: msg.from,
      taskId: msg.taskId ?? null
