...
export function validateTaskBrief(taskBrief) {
  ...
  for (const field of REQUIRED_FIELDS) {
    // 特殊处理 completion_criteria / completionCriteria 兼容性
    if (field === 'completion_criteria') {
      if (!taskBrief.completion_criteria && !taskBrief.completionCriteria) {
        errors.push(`缺少必填字段: ${field} 或 completionCriteria`);
      }
      continue;
    }
...
export function formatTaskBrief(taskBrief) {
  ...
  // 完成标准
  const completionCriteria = taskBrief.completionCriteria || taskBrief.completion_criteria;
