...
export class ContentRouter {
  ...
  async routeFileContent(path, serviceId, workspaceId = null) {
    try {
      ...
      const fileInfo = await getWorkspaceManager().getFileInfo(workspaceId, path);
      if (!fileInfo) {
        return {
          contentType: 'text',
          routing: 'text',
      ...
      if (!fileInfo.isBinary) {
        ...
        const readResult = await ws.readFile(path, { length: 5000 });
        return {
          contentType: 'text',
          routing: 'text',
      ...
      // 根据二进制类型和能力路由
      if (binaryType === 'image') {
        return await this._routeImageContent(fileInfo, serviceId, binaryType, workspaceId ?? undefined);
      } else {
        return await this._routeNonImageBinaryContent(fileInfo, serviceId, binaryType, workspaceId ?? undefined);
      }
    } catch (error) {
      void this.logger.error('路由文件内容时出错', { path, serviceId, error: error.message, stack: error.stack, name: error?.name, code: error?.code });
      return {
        contentType: 'text',
        routing: 'text',
  ...
  async routeMessageContent(message, serviceId, options = {}) {
    ...
    if (supportedImages.length > 0) {
      const getImageBase64 = async (path) => {
        const ws2 = await getWorkspaceManager().getWorkspace(workspaceId);
        const result = await ws2.readFile(path, { encoding: 'base64' });
  ...
  formatImageUrl(base64Content, mimeType) {
    ...
    // ai-sdk image part format: { type: "image", image: dataContent | URL }
    return {
      type: 'image',
      image: `data:${normalizedMime};base64,${base64Content}`
    };
  }
...
注意: 创建角色时必须指定支持 ${requiredCapability} 能力的。`;
   ...
   * @param {string} binaryType - 二进制类型
   * @param {string|undefined} [workspaceId] - 工作区ID
   * @returns {Promise<ContentRouteResult>} 路由结果
   */
  async _routeImageContent(fileInfo, serviceId, binaryType, workspaceId) {
    const hasVision = this.hasCapability(serviceId, 'vision');
    
    if (hasVision) {
      const ws3 = await getWorkspaceManager().getWorkspace(workspaceId);
      const result = await ws3.readFile(fileInfo.path, { encoding: 'base64' });
   ...
   * @returns {Promise<ContentRouteResult>} 路由结果
   */
  async _routeNonImageBinaryContent(fileInfo, serviceId, binaryType, workspaceId) {
  ...
  async _processFileAttachments(attachments, workspaceId = null) {
    ...
    for (const att of attachments) {
      ...
      try {
        const ws4 = await getWorkspaceManager().getWorkspace(workspaceId);
        const fileData = await ws4.readFile(path, { length: 50000 });
