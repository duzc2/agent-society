...
export const BUILTIN_TOOL_GROUPS = {
  ...
  workspace: {
    ...
    tools: ["file_read_lines", "file_search", "file_line_count", "edit_file", "replace_file", "append_file", "list_files", "get_workspace_info", "search_text"]
  },
  command: {
...
export class ToolGroupManager {
  ...
  getToolDefinitions(groupIds) {
    ...
    for (const groupId of groupIds) {
      const group = this._groups.get(groupId);
      if (!group) {
        void this.log.warn("工具组不存在", { groupId, requestedGroupIds: groupIds });
        continue;
      }
