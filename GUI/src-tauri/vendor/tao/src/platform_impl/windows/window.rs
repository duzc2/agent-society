...
unsafe fn init<T: 'static>(
  ...
  // PATCH(Agent Society GUI): transparent windows must be created with
  // WS_EX_NOREDIRECTIONBITMAP (set after creation is silently ignored by
  // Windows, and the DWM blur-behind fallback renders white on Win11).
  // Upstream tao leaves this to the caller, but tauri never passes it.
  window_flags.set(
