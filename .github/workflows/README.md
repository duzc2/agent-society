...
# 指定自定义阈值
node scripts/check-coverage.js --threshold=85

# 指定 LCOV 文件路径
node scripts/check-coverage.js --lcov-file=coverage/lcov.info
```
...
缓存键基于 `package-lock.json` 文件的哈希值，确保依赖变更时重新安装。
...
1. 运行 `node --test --experimental-test-coverage` 查看详细报告
2. 识别未覆盖的代码
3. 添加测试用例
4. 重新运行测试
...
- [Node.js 测试文档](https://nodejs.org/docs/latest/api/test.html)
