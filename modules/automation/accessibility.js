...
export class AccessibilityService {
  ...
  async _runPSScript(scriptContent, timeout = 30000) {
    ...
    const { stdout, stderr } = await execAsync(
      ...
      { timeout, encoding: 'buffer' }
    );
    return {
      stdout: this._decodeBuffer(stdout),
      stderr: this._decodeBuffer(stderr),
      success: true
    };
  }
  ...
  async findControl(criteria) {
    try {
      ...
      // Map lowercase controlType enum values to PascalCase ProgrammaticName suffixes
      const PSLiteral = v => v.replace(/'/g, "''");
      const TYPE_MAP = {
        button: 'Button', edit: 'Edit', window: 'Window', pane: 'Pane',
        document: 'Document', list: 'List', listItem: 'ListItem',
        tree: 'Tree', treeItem: 'TreeItem', tab: 'Tab', tabItem: 'TabItem',
        menu: 'Menu', menuItem: 'MenuItem', toolBar: 'ToolBar',
        statusBar: 'StatusBar', comboBox: 'ComboBox', checkBox: 'CheckBox',
      ...
      if (criteria.controlType) {
        const suffix = TYPE_MAP[criteria.controlType] || criteria.controlType;
        checks.push(`$_.Current.ControlType.ProgrammaticName -like "*${PSLiteral(suffix)}"`);
      }
      if (criteria.name) {
...
if ($found) {
    ...
    $ct = ($found.Current.ControlType.ProgrammaticName -replace '\\|', '_') -replace ':', '_'
    $nm = ($found.Current.Name -replace '\\|', '_') -replace ':', '_'
    $id = ($found.Current.AutomationId -replace '\\|', '_') -replace ':', '_'
    $cn = ($found.Current.ClassName -replace '\\|', '_') -replace ':', '_'
    $output = "FOUND:$ct|$nm|$id|$cn|" + [int]$rect.X + "|" + [int]$rect.Y + "|" + [int]$rect.Width + "|" + [int]$rect.Height
    Write-Host $output
} else {
    Write-Host "NOT_FOUND"
      ...
      if (resultLine && resultLine.startsWith('FOUND:')) {
        // 格式: FOUND:ControlType.PascalName|Name|AutomationId|ClassName|X|Y|Width|Height
        const parts = resultLine.slice(6).split('|');
        if (parts.length >= 8) {
          return {
            ok: true,
            found: true,
...
if ($target) {
  ...
  async waitForControl(criteria, timeout = 10000) {
    ...
    while (Date.now() - startTime < timeout) {
      const result = await this.findControl(criteria);
      if (result.ok && result.found) {
        return { ok: true, found: true, control: result.control };
      }
  ...
  async screenshotControl(ctx, criteria, destPath, options = {}) {
    ...
    const rootWorkspaceId = this.runtime.findWorkspaceIdForAgent(agentId);
    if (!rootWorkspaceId) {
    ...
    const workspace = await getWorkspaceManager().getWorkspace(rootWorkspaceId);
