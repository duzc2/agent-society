...
export class FileService {
  ...
  async writeFile(ctx, filePath, content, options = {}) {
    try {
      ...
      if (!existsSync(dir)) {
        // 检查是否有权限创建目录
        const dirOrgId = this._getOrgId(ctx);
        const dirPermission = await this.permissionManager.checkWritePermission(dir, dirOrgId);
  ...
  async _getWorkspacePath(ctx) {
    try {
      ...
      if (wsId) {
        ...
        if (wm.checkWorkspaceExists(wsId)) {
          return wm.getWorkspacePath(wsId);
        }
      }

      // 备选：没有分配 workspace 时，使用 taskId 创建
      const taskId = ctx?.currentMessage?.taskId;
      if (taskId) {
        const wm = getWorkspaceManager();
        await wm.createWorkspace(taskId);
        return wm.getWorkspacePath(taskId);
      }

      return null;
    } catch (error) {
      this.log.error("[LocalFile] 获取工作区路径失败", {
        error: error.message,
        stack: error.stack,
      });
      return null;
  ...
  /**
   * 从 context 中提取组织 ID。
   * 通过 runtime.findWorkspaceIdForAgent 沿祖先链查找，
   * 返回 root 的直接子智能体 ID（即组织 ID）。
   * @private
   * @param {object} ctx
   * @returns {string|null}
   */
  _getOrgId(ctx) {
    ...
    if (!runtime?.findWorkspaceIdForAgent) return null;
    return runtime.findWorkspaceIdForAgent(agentId);
  }
}
