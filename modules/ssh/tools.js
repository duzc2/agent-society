...
export function getToolDefinitions() {
  return [
    ...
    },
    {
      ...
      function: {
        name: 'ssh_shell_send',
        description: '在shell会话中发送命令（异步，立即返回）使用 ssh_shell_read 读取输出',
        parameters: {
          type: 'object',
          properties: {
            shellId: {
              type: 'string',
              description: '会话ID（由ssh_shell_create返回的纯字符串ID，如 shell_1234567890_1，不需要添加引号）'
            },
            command: {
              type: 'string',
              description: '要执行的命令'
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            ...
            offset: {
              type: 'number',
              description: '文件偏移位置（字节数），注意要自己维护好这个值，不要错过重要信息，也不要停在原地读取。'
            }
          },
    ...
    },
    {
      ...
      function: {
        name: 'ssh_shell_list',
        description: '列出当前智能体已创建的所有shell会话。返回结构：{ ok: true, shells: [{ shellId: "会话ID", hostName: "主机名", connectionId: "连接ID", isActive: true/false, createdAt: "创建时间" }] }',
        parameters: {
          type: 'object',
          properties: {},
          required: []
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            taskId: {
              type: 'string',
              description: '任务ID（由ssh_upload或ssh_download返回的纯字符串ID，不需要添加引号）'
            }
          },
          required: ['taskId']
        }
