...
- `connectionId`: 底层连接ID
- `isActive`: 会话是否处于活动状态
- `createdAt`: 会话创建时间（ISO格式）

**错误码**：
- `list_shells_failed`: 列出会话失败

**说明**：
- 仅返回**当前请求的智能体**创建的会话，不会返回其他智能体的shell
- 可用于查看自己打开的shell列表，避免重复创建
- 多个shell可以共用同一个底层连接
- 每个shell在创建时都会记录所属智能体ID，列表时按此过滤
