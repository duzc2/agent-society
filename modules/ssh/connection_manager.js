  ...
  _startIdleCleanup() {
    ...
    this._idleCleanupTimer = setInterval(() => {
      try {
        ...
        this.log.warn('[ConnectionManager] 空闲连接清理检查时出错', { error: e.message, stack: e.stack });
      }
    }, interval);
    this._idleCleanupTimer?.unref?.();
  }
          ...
          if (conn?._onDisconnect) {
            for (const callback of conn._onDisconnect) {
              try { callback(); } catch (e) {
                this.log.warn('[ConnectionManager] 断开连接回调执行失败', { error: e.message, stack: e.stack });
              }
            }
          }
