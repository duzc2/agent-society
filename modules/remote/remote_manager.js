...
`.trim();
   ...
   *
   * 远程进程的生命周期管理已统一交由 localcmd：remote 只负责把 SSH channel
   * 桥接为"进程样对象"（见 modules/remote/index.js 的 _execRemoteChannel），
   * 日志写入、解码、live 状态、历史查询、终止语义全部由 localcmd 的
   * ProcessManager 处理，与本方法无关。
   * @param {string} command
   * @param {string[]} [args]
   * @param {string} [cwd]
   * @param {object} [env]
   * @returns {string}
   */
  buildRemoteShellCommand(command, args, cwd, env) {
