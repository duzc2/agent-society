...
class FileViewerService {
  ...
  async getFile(workspaceId: string, filePath: string): Promise<FileContent> {
    ...
    // 特殊处理：如果文件内容实际上是文本，但 Content-Type 被误判为二进制流
    // 可以尝试通过文件扩展名进行辅助判断
    const ext = this.getFileExtension(filePath);
    const textExtensions = [
      'md', 'markdown', 'txt', 'log', 'csv', 'json', 'jsonl', 'ndjson', 'js', 'ts', 'jsx', 'tsx', 'vue',
      'css', 'scss', 'sass', 'less', 'html', 'htm', 'xml', 'yaml', 'yml',
      'py', 'java', 'c', 'cpp', 'h', 'hpp', 'go', 'rs', 'rb', 'php', 'sh', 'bat', 'cmd',
   ...
   * @param filePath - 文件路径
   * @param content - 文件内容
   * @param mimeType - 文件类型（可选）
   */
  async saveFile(workspaceId: string, filePath: string, content: string, mimeType?: string): Promise<void> {
    const url = `/api/workspaces/${encodeURIComponent(workspaceId)}/file?path=${encodeURIComponent(filePath)}`;
    ...
    const response = await fetch(url, {
      ...
      body: JSON.stringify({
        ...
        mimeType,
        operator: 'user', // 标记操作者
        messageId: 'manual-save' // 标记来源
      })
    });

    if (!response.ok) {
      const errorData = await response.json().catch(() => ({}));
      throw new Error(errorData.error || `保存文件失败: HTTP ${response.status}`);
    }
  }
