...
import { Sparkles, Users, ArrowRight, Send, Loader2, X, ChevronDown, ChevronRight, Archive } from 'lucide-vue-next';
...
// 获取除了首页之外的所有组织，按状态分组
const activeOrganizations = computed(() => orgStore.orgs.filter(o => o.id !== 'home' && !isOrgDeleted(o.id)));
const deletedOrganizations = computed(() => orgStore.orgs.filter(o => o.id !== 'home' && isOrgDeleted(o.id)));
  ...
  <div class="h-full overflow-y-auto bg-[var(--bg)] p-8">
    <div class="max-w-6xl mx-auto space-y-12">
      ...
      <section class="space-y-6">
        ...
        <!-- 已删除的组织（折叠区域） -->
        <div v-if="deletedOrganizations.length > 0" class="mt-4 border-t border-[var(--border)] pt-4">
          <button
            class="flex items-center space-x-2 px-2 py-1 text-sm text-[var(--text-3)] hover:text-[var(--text-2)] transition-colors group"
          ...
          <div v-if="showDeletedOrgs" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 mt-3">
            <Card
              v-for="org in deletedOrganizations"
              :key="org.id"
              class="!bg-[var(--surface-1)] !border-[var(--border)] hover:!border-[var(--primary)] hover:shadow-lg transition-all cursor-pointer group relative overflow-hidden opacity-60 hover:opacity-100"
              @click="handleOrgClick(org)"
            >
              <template #content>
                <div class="flex flex-col items-start text-left space-y-4 pt-2">
                  <div class="w-16 h-16 rounded-2xl bg-[var(--surface-3)] flex items-center justify-center text-2xl font-bold text-[var(--text-3)] group-hover:rotate-6 transition-transform">
                    {{ org.initial }}
                  </div>
                  <div class="flex-grow min-w-0 w-full">
                    <div class="flex items-center justify-between">
