            ...
            <div v-for="role in overview.roles" :key="role.roleId" class="rounded-lg border border-[var(--border)] bg-[var(--surface-1)] p-3">
              <div class="flex items-center justify-between gap-2">
                <div class="min-w-0">
                  ...
                  <div class="mt-1 text-xs text-[var(--text-3)]">{{ role.roleId }}</div>
                </div>
            ...
            <div v-for="agent in overview.agents" :key="agent.agentId" class="rounded-lg border border-[var(--border)] bg-[var(--surface-1)] p-3">
              <div class="flex items-center justify-between gap-2">
                <div class="min-w-0">
                  ...
                  <div class="mt-1 text-xs text-[var(--text-3)]">{{ agent.roleName }} · {{ agent.agentId }}</div>
                </div>
...
const load = async () => {
  ...
  loading.value = true;
  try {
    overview.value = await skillApi.getSkillOverview(props.skillId);
  } finally {
    loading.value = false;
  }
};
