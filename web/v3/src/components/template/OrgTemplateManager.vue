...
const pendingTemplate = ref<OrgTemplate | null>(null);
...
const handleSwitchConfirmed = () => {
  if (pendingTemplate.value) {
    selectedTemplateId.value = pendingTemplate.value.id;
  ...
  <div class="flex flex-col h-full bg-[var(--surface-1)] text-[var(--text-1)]">
    ...
    <div class="flex flex-1 overflow-hidden">
      ...
      <div class="w-64 flex flex-col border-r border-[var(--border)] bg-[var(--surface-2)]">
        ...
        <!-- 模板列表 -->
        <ScrollPanel v-else class="flex-1" style="height: 0">
    ...
    <!-- 切换模板确认对话框 -->
    <ConfirmDialog
      v-model:visible="showSwitchConfirm"
      title="切换模板"
      message="当前修改未保存，切换模板将丢失修改，是否继续？"
      confirm-label="继续"
      cancel-label="取消"
      confirm-severity="danger"
      @confirm="handleSwitchConfirmed"
    />
  </div>
</template>
