...
功能组件（服务、管理器、注册表、提供器等注入的依赖对象）不得做空值兼容，不得使用 `?? null` 默认值，不得使用 `if` 进行空值判断。如果组件依赖为空，应直接报错，不能掩盖问题。

只有配置参数（字符串、布尔值、数值、阈值等）可以做空值兼容。

反例：
```js
// 错误：对依赖对象做空值兼容
this._lifecycleRegistry = options.lifecycleRegistry ?? null;
if (this._lifecycleRegistry) {
...
1. 保证创建了对应的测试代码，并运行 node --test 测试
2. 无论通过与否，停下来
3. 向用户报告结果
4. **绝对不允许**在用户明确许可前继续下一步操作
5. 绝对不允许操作git，不允许自动提交，不允许自动推送，不允许自动恢复文件，所有的git操作必须停下来问用户。
